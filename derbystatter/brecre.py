                ...
                for ps in range(2, 9):
                    ...
                    if passpts is None:
                        if ps == 2:
                            if not sk.JamNP(i):
                                Error(
                                    "Jam", i, "Pass", ps,
                                    "has no recorded points, but NP not checked"
                                )
                        continue
                    if sk.JamNP(i):
                        if ps == 2 and (passpts == '' or passpts != "-"):
                            Error(
                                "Jam", i, "Pass", ps,
                                "should be '-' or blank if NP checked"
                            )
                    if sk.version.hasGhostPoints:
                        ...
                        if len(ghostpts) > passpts:
                            Error(
                                "Jam", i, "Pass", ps,
                                "has more ghost points than recorded points"
                            )
                        if passpts == 5:
                            if len(ghostpts) == 0:
                                Warning(
                                    "Jam", i, "Pass", ps,
                                    "has 5 points, but no recorded ghost points"
                                )
                            elif (not "J" in ghostpts
                                 ) and (not "L" in ghostpts):
                                Error(
                                    "Jam", i, "Pass", ps,
                                    "has 5 points, but no Jammer or Lap ghost points, recorded",
                                    ghostpts
                                )
            ...
            for i in range(0, len(team.skaters)):
                ...
                for pi in range(firstMajor[i], lastMajor[i]):
                    ...
                    if penalty[0] == '4' or penalty[0] == 4:
                        ...
                        CheckPenaltyCode(
                            penalty[0], team.skaters[i].number, True
                        )
                    if verbose:
                        print "[", penalty[0], "/", penalty[1], "]",
                if verbose:
                ...
                if len(fourth) > len(fours):
                    Error(
                        "More fourth minors (" + str(len(fourth)) +
                        ") than served (" + str(len(fours)) + "), skater " +
                        team.skaters[i].number,
                    )
                if len(fourth) < len(fours):
                    Error(
                        "Fewer fourth minors (" + str(len(fourth)) +
                        ") than served (" + str(len(fours)) + "), skater " +
                        team.skaters[i].number,
                    )
                for jamnum in fourth:
                    if jamnum in fours:
                        ...
                        Error(
                            "Fourth Minor occured in jam",
                            jamnum,
                            "but no corresponding penalty minute served for skater",
                            team.skaters[i].number,
                        )
                ...
                for skater in skaters:
                    ...
                    if startsInBox(box[0][0]):
                        if skater not in lastSkaters:
                            ...
                            Error(
                                skater, "started jam", i,
                                "in the box, but wasn't in previous jam"
                            )
                lastSkaters = skaters
        ...
        for period in (0, 1):
            ...
            for i in range(1, book.version.maxNumJams):
                ...
                if skaters[4] != sk.Jammer(i):
                    Error(
                        "Jam", i, "has", skaters[4], "as jammer in lineup, but",
                        sk.Jammer(i), "in score"
                    )
            ...
            for i in range(0, len(team.skaters)):
                ...
                for pi in range(0, 28):
                    ...
                    if skaters and skNum not in skaters:
                        Error(
                            "Skater", skNum, "got minor penalty (", penalty[0],
                            ") in jam", jamNumber,
                            "but not in lineup for that jam"
                        )
                for pi in range(0, 7):
                    penalty = pt.MajorPenalty(i, pi)
                    if not penalty:
                    ...
                    if skaters and skNum not in skaters:
                        Error(
                            "Skater", skNum, "got major penalty (", penalty[0],
                            ") in jam", jamNumber,
                            "but not in lineup for that jam"
                        )
...
def main():
    ...
    if not book.HasLineupSheets() and book.HasPenaltySheets(
    ) and book.HasScoreSheets():
