    ...
    def Config(self, obj):
        ...
        configs = {
            ...
            'PenaltyTrackerNoMinors/Official March 2014 Release': {
                'BaseRow': 3,
                'MajorPenaltyCount':
                7,  # really 9, but not everything supports that yet
...
class StatBook:
    ...
    def __init__(self, path):
        ...
        jt_names = [
            self.version.periodTimerSheetName,
            self.version.periodTimerSheetName.replace(' ', '_')
        ]
        for name in jt_names:
            try:
                self.JamTimer = self.workbook.sheet_by_name(name)
                self.jamTimer = (
                    JamTimer(self.JamTimer, 0, 0),
                    JamTimer(self.JamTimer, 0, 1)
                )
                break
            except KeyError:
                pass
    ...
    def BoutInfo(self):
        ...
        if type(date) == type("") or type(date) == type(
                u""
    ...
    def TitleSummary(self):
        ...
        return bi[
            'date'
        ] + ":" + self.home.teamName + "(" + "%d" % self.home.TotalScore(
        ) + ") vs " + self.away.teamName + "(" + "%d" % self.away.TotalScore(
        ) + ")"
    ...
    def HasLineupSheets(self):
        ...
        if self.version.hasNoPivot:  # 2012 Oct version auto include the jam number and jammer skater
            ...
            for ji in range(0, 1):
                if not IsBlankCell(
                        self.LineUp.cell(
                            1 + self.version.headerRows + ji * 2, 5
                        )
                ):
                    return True
                if not IsBlankCell(
                        self.LineUp.cell(
                            1 + self.version.headerRows + ji * 2, 8
                        )
                ):
                    return True
                if not IsBlankCell(
                        self.LineUp.cell(
                            1 + self.version.headerRows + ji * 2, 11
                        )
                ):
                    return True
                if not IsBlankCell(
                        self.LineUp.cell(
                            1 + self.version.headerRows + ji * 2, 14
                        )
                ):
                    return True
            return False
...
class Team:
    ...
    def __init__(self, book):
        ...
        for i in range(0, 20):
            ...
            if IsBlankCell(
                    num
            ) or num.value == "-":  # "For any empty spots in either roster, input "-" in Skater # and Skater Name columns."
                break
        ...
        self.scorekeeper = (
            Scorekeeper(self.statbook.Score, self.TeamIndex, 0),
            Scorekeeper(self.statbook.Score, self.TeamIndex, 1)
        )
        if self.statbook.version.hasMinors:
            self.penaltyTracker = (
                PenaltyTracker(self.statbook.Penalty, self.TeamIndex, 0),
                PenaltyTracker(self.statbook.Penalty, self.TeamIndex, 1)
            )
        else:
            self.penaltyTracker = (
                PenaltyTrackerNoMinors(
                    self.statbook.Penalty, self.TeamIndex, 0
                ), PenaltyTrackerNoMinors(
                    self.statbook.Penalty, self.TeamIndex, 1
                )
            )
        if book.HasLineupSheets():
            self.lineupTracker = (
                LineupTracker(self.statbook.LineUp, self.TeamIndex, 0),
                LineupTracker(self.statbook.LineUp, self.TeamIndex, 1)
            )
        else:
            self.lineupTracker = (
                EstLineupTracker(
                    self, self.scorekeeper[0], self.penaltyTracker[0]
                ), EstLineupTracker(
                    self, self.scorekeeper[1], self.penaltyTracker[1]
                )
            )
        if book.version.hasActionsErrors:
            self.actions = (
                Actions(self.statbook.Actions, self.TeamIndex, 0),
                Actions(self.statbook.Actions, self.TeamIndex, 1)
            )
            self.errors = (
                Errors(self.statbook.Errors, self.TeamIndex, 0),
                Errors(self.statbook.Errors, self.TeamIndex, 1)
            )
        else:
            self.actions = ()
            self.errors = ()
        self.league = self.statbook.IBRF.cell(
            self.IGRFLeagueRow, self.IBRFCol
        ).value
        self.team = self.statbook.IBRF.cell(
            self.IGRFTeamRow, self.IBRFCol
        ).value
    ...
    def Period(self, index):
        ...
        index = index - 1
        return (
            self.scorekeeper[index], self.penaltyTracker[index],
            self.lineupTracker[index], self.actions[index], self.errors[index]
        )

    def SkaterNum(self, index):
        """Returns the number for the given skater in the IBRF"""
        return self.statbook.IBRF.cell(
            index + self.IGRFSkaterFirstRow, self.IBRFCol
        )

    def SkaterName(self, index):
        """Returns the name for the given skater in the IBRF"""
        return self.statbook.IBRF.cell(
            index + self.IGRFSkaterFirstRow, self.IBRFCol + 1
        )
    ...
    def TotalScore(self, period=None):
        ...
        """
        if period is None:
            return self.scorekeeper[0].TotalScore() + self.scorekeeper[
                1].TotalScore()
    ...
    def TotalMajors(self, period=None):
        ...
        """
        if period is None:
            return self.penaltyTracker[0].TotalMajors() + self.penaltyTracker[
                1].TotalMajors()
    ...
    def EstimateLineUp(self):
        ...
        self.lineupTracker = (
            EstLineupTracker(self, self.scorekeeper[0], self.penaltyTracker[0]),
            EstLineupTracker(self, self.scorekeeper[1], self.penaltyTracker[1])
        )
...
class Skater:
    ...
    def __repr__(self):
        if self.name:
            return self.team.teamName + u" " + unicode(
                self.number
            ) + u"(" + unicode(self.name) + u")"
        else:
...
class PeriodStat:
    ...
    def relcell(self, drow, dcol):
        ...
        return self.sheet.cell(
            self.BaseRow + self.RowsPerTeam * self.teamIndex +
            self.RowsPerPeriod() * self.period + drow,
            self.BaseCol + self.ColsPerTeam * self.teamIndex +
            self.ColsPerPeriod * self.period + dcol
        )
...
class Scorekeeper(PeriodStat):
    ...
    def JamPoints(self, jam, starpass=False):
        ...
        if ji is None:
            ...
            return None
        return self.relcell(
            ji + self.RowsPerJam * starpass,
            7 + self.ColsPerPass * (self.version.maxNumPasses - 1)
        ).value
    ...
    def JamPassPoints(self, jam, pss, starpass=False):
        ...
        if ji is None:
            return None
        cell = self.relcell(
            ji + self.RowsPerJam * starpass, 7 + (pss - 2) * self.ColsPerPass
        )
        if IsBlankCell(cell):
            return None
        return cell.value
    ...
    def JamPassGhostPoints(self, jam, pss, starpass=False):
        ...
        v = self.relcell(
            ji + self.RowsPerJam * starpass,
            7 + (pss - 2) * self.ColsPerPass + 1
        ).value
        if v:
            retval.append(v)
        v = self.relcell(
            ji + self.RowsPerJam * starpass,
            7 + (pss - 2) * self.ColsPerPass + 2
        ).value
        if v:
            retval.append(v)
        v = self.relcell(
            ji + self.RowsPerJam * starpass + 1,
            7 + (pss - 2) * self.ColsPerPass + 1
        ).value
        if v:
            retval.append(v)
...
class LineupTracker(PeriodStat):
    ...
    def LineupForJam(self, jam, starpass=False):
        ...
        if IsBlankCell(jammer):
            return None
        jammer = jammer.value
        pivot = self.relcell(ji,
                             firstCol + self.version.lineupColumns['p']).value
        blocker2 = self.relcell(ji, firstCol + self.version.lineupColumns['2']
                               ).value
        blocker3 = self.relcell(ji, firstCol + self.version.lineupColumns['3']
                               ).value
        blocker4 = self.relcell(ji, firstCol + self.version.lineupColumns['4']
                               ).value
        return (
            CanonSkaterNumber(pivot), CanonSkaterNumber(blocker2),
            CanonSkaterNumber(blocker3), CanonSkaterNumber(blocker4),
            CanonSkaterNumber(jammer)
        )
    ...
    def PenaltyPasses(self, jam, starpass=False):
        ...
        for si in range(0, 5):
            sn = self.relcell(
                ji + self.RowsPerJam * starpass,
                firstCol + self.ColsPerSkater * si
            )
            if IsBlankCell(sn):
            ...
            if self.RowsPerJam == 1:
                for pi in range(0, self.ColsPerSkater - 1):
                    code = self.relcell(
                        ji + self.RowsPerJam * starpass,
                        firstCol + self.ColsPerSkater * si + 1 + pi
                    )
                    if IsBlankCell(code):
                        continue
                    if code.value == 'S':
                        ...
                        retval[sn].append((None, 1)
                                         )  # start in box, exit after star pass
                    elif code.value == '\\':
                        retval[sn].append(
                            (None, 1)
                        )  # entered in jam, exit after star pass
                ...
                for pi in range(0, 1):
                    startPass = self.relcell(
                        ji + pi + 2 * starpass,
                        firstCol + self.ColsPerSkater * si + 1
                    )
                    endPass = self.relcell(
                        ji + pi + 2 * starpass,
                        firstCol + self.ColsPerSkater * si + 2
                    )
...
class JamTimer(PeriodStat):
    ...
    def JamDuration(self, jam):
        ...
        if jamDuration > 0.5:  # 2:00 is 12:02 PM
            jamDuration = round((jamDuration - 0.5) *
                                (24 * 60 * 60))  # fraction of a day
        else:
...
class Actions(PeriodStat):
    ...
    def AssistsForSkater(self, skater):
        ...
        return [
            self.relcell(skater, 0).value, self.relcell(skater, 1).value,
            self.relcell(skater, 2).value, self.relcell(skater, 3).value,
